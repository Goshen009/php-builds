...
use Core\{Router, Filter, FlashMessages};
use App\Controllers\{AuthController, BookController, UserController, ViewController};
use App\Repositories\{BookRepository, UserRepository};

function load_routes(Router $router, FlashMessages $flashMessages, Filter $filter, PDO $db) {
    $userRepository = new UserRepository($db);
    $bookRepository = new BookRepository($db);

    $authController = new AuthController($db, $userRepository, $router, $filter);
    $bookController = new BookController($db, $bookRepository, $router, $filter);
    $userController = new UserController($db, $userRepository, $router, $filter);
    $viewController = new ViewController($router, $flashMessages);
